...
const TablasAdmin = (props) => {
  const botonesAcciones = (params, nombre) => {
    ...
    const handleDelete = (nombre) => {
      switch(nombre){
        case 'estudiantes':
          ...
          .then((response) => {
            navigate('/confirmacioncrearusuario', { state: { mensaje: '¡Alumno eliminado con éxito!' } });
            })
        ...
        case 'profesores':
          ...
          .then((response) => {
            navigate('/confirmacioncrearusuario', { state: { mensaje: '¡Profesor eliminado con éxito!' } });
            })
            .catch((error) => console.error('Error al eliminar:', error));
          break;
        case 'tareas':
          ...
          .then((response) => {
            navigate('/confirmacioncrearusuario', { state: { mensaje: '¡Tarea eliminada con éxito!' } });
            })
            .catch((error) => console.error('Error al eliminar:', error));
          break;
        case 'clases':
    ...
    const handleEdit = (nombre) => {
      switch(nombre){
        case 'estudiantes':
            navigate('/admin/editaralumno', { state: { id: params.row.id }})

          break;
        case 'profesores':
          navigate('/admin/editarprofe', { state: { id: params.row.id }})
          break;
        case 'tareas':
          axios.delete(`http://localhost:5050/tareas/borrarTarea/${params.row.id}`)
          .then((response) => {
    ...
    return(
      ...
      <IconButton
        variant="outlined"
        color="primary"
        onClick={() => handleEdit(nombre, params.row)}
      >
        <EditIcon/>
      </IconButton>
      <IconButton
      ...
      <Dialog open={bdialogAbierto} onClose={() => setbDialogAbierto(false)}>
        ...
        <DialogActions>
          ...
          <Button onPress={() => handleDelete(nombre, params.row)} title='Eliminar' color='#b71c1c'/>
        </DialogActions>
        </Dialog>
      </div>
    );
                ...
                switch(props.nombre){
                  case 'estudiantes':
                      return {
                        ...
                        f_nac: dayjs(fila.fecha_nac).format('YYYY/MM/DD'),
  ...
  const toolBar = (props) => { // Toolbar custom del panel de DataGrid
    ...
    // Función para renderizar la celda de la columna 'admin'
  const renderAdminCell = (params) => {
    const isAdmin = params.row.admin === 1;
    return isAdmin ? 'SI' : 'NO';
  };

  // Modificar la columna 'admin' para utilizar la función renderAdminCell
  const columnasModificadas = columnas.map((columna) => {
    if (columna.field === 'admin') {
      return { ...columna, renderCell: renderAdminCell };
    }
    return columna;
  });
    return(
      <Box>
        <DataGrid rows={filas} columns={columnasModificadas}
        slots={{
