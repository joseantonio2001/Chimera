...
app.post('/api/crearAlumno', (req, res) => {
  ...
  const query1 =
    'INSERT INTO usuarios (dni, nombre, apellido1, apellido2, fecha_nac, PASSWORD,img_perfil) VALUES (?, ?, ?, ?, ?, ?,?)';
...
app.get('/api/getUsuarios', (req, res) => {
  console.log('El servidor está preparado!');
  const query =
    'SELECT * FROM usuarios LEFT JOIN profesores ON usuarios.id = profesores.id LEFT JOIN estudiantes ON usuarios.id = estudiantes.id';
